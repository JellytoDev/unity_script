...
//		skirtNum = Random.Range( 0, arrSkirts.Count );
	...
	void OnCollisionEnter(Collision other)
	{
		if (other.transform.tag == "shop")
		{
			...
			DataInfo.GameData.shopName = other.transform.name;
			Application.LoadLevel("clothing_store"); 
		}else if (other.transform.tag == "dressing_room")
		{
	...
	private void OnCollisionStay(Collision other)
	{
		...
		if (other.transform.tag == "tshirt")
		{
			if (Input.GetKeyDown(KeyCode.Space))
			{
				DataInfo.shopUI.shopTag = other.transform.tag;
				GameObject.Find("ShopBox").transform.FindChild("ShopUI").gameObject.SetActive(true);
				Debug.Log("tshirt 스페이스 키 누름");
			}
		}
		else if (other.transform.tag == "cap")
		{
			if (Input.GetKeyDown(KeyCode.Space))
			{
				DataInfo.shopUI.shopTag = other.transform.tag;
				GameObject.Find("ShopBox").transform.FindChild("ShopUI").gameObject.SetActive(true);
				Debug.Log("cap 스페이스 키 누름");
			}
		}
	}
